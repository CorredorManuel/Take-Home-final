...
for(i in 1:filas){
  ...
  verificarse #este seria el valor a verificar conla distribucion T
  ...
  if(verificarse[i]>izquierda & verificarse[i]< derecha) { #alta potencia
    resultado[i] <-  "No se puede rechazar que el coeficiente sea 0"
  }else{
    resultado[i] <- "Se puede rechazar que el coeficiente sea O"
  }
